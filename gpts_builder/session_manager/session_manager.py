...
from session_manager.storage.global_storage import GlobalStorage
from session_manager.storage.redis_storage import RedisStorage
...
class SessionManager(object):
    ...
    def build_session(self, session_id, system_prompt=None):
        ...
        logger.info(f"self.global_map {self.session_storage.global_map}")

        logger.info(f"session_id {session_id} session_json {session_json}")
        if session_json is None:
            ...
            logger.info(f"self.sessioncls.from_json {session}")
